...
export interface TestBlock {
    name: string;
    startLine: number; // 0-based line index of the `With test(...)` line
    endLine: number;   // 0-based line index of the matching `EndWith` line
}
...
function stripLineComment(line: string): string {
    ...
    for (let i = 0; i < line.length; i++) {
        ...
        if (inString) {
            if (c === quote) {
                if (line[i + 1] === quote) { i++; continue; } // doubled quote = literal, stay in string
                inString = false;
                quote = '';
            }
        } else if (c === '"' || c === "'") {
...
export function findTests(text: string): TestBlock[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inBlockComment && /^#(cs|comments-start)\b/i.test(code)) { inBlockComment = true; continue; }
        if (inBlockComment) {
            if (/^#(ce|comments-end)\b/i.test(code)) { inBlockComment = false; }
            continue;
        }
        if (code === '') { continue; }
        ...
        if (/^With\b/i.test(code)) {
            if (depth === 0) {
                ...
                if (m) { current = { name: m[2], startLine: i }; }
            }
            depth++;
...
function findTopLevelExitLines(text: string): number[] {
    const lines = splitLines(text);
    const result: number[] = [];
    let withDepth = 0;
    let funcDepth = 0;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^Func\b/i.test(code)) { funcDepth++; }
        else if (/^EndFunc\b/i.test(code)) { funcDepth = Math.max(0, funcDepth - 1); }
        else if (/^With\b/i.test(code)) { withDepth++; }
        else if (/^EndWith\b/i.test(code)) { withDepth = Math.max(0, withDepth - 1); }
    }
    return result;
}
...
function findFuncRanges(text: string): LineRange[] {
    const lines = splitLines(text);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (code === '') { continue; }
        if (start < 0 && /^Func\b/i.test(code)) { start = i; }
        else if (start >= 0 && /^EndFunc\b/i.test(code)) { ranges.push({ start, end: i }); start = -1; }
    }
    return ranges;
}
...
export function buildSingleTestScript(text: string, keepName: string): string | null {
    ...
    const exitLines = new Set<number>(findTopLevelExitLines(text));
