...
connection.onHover((params) => {
    ...
    let dirStart = start;
    while (dirStart > 0 && /[A-Za-z0-9_-]/.test(text[dirStart - 1]))
        dirStart--;
    if (dirStart > 0 && text[dirStart - 1] === '#') {
        let dirEnd = end;
        while (dirEnd < text.length && /[A-Za-z0-9_-]/.test(text[dirEnd]))
            dirEnd++;
    ...
    function signatureMarkdown(overloads) {
        return overloads.map(fn => '```autoit\n' + buildSignatureLabel(fn) + '\n```' + (fn.documentation ? '\n' + fn.documentation : '')).join('\n\n---\n\n');
    }
    ...
    if (prev === '.') {
        ...
        if (!overloads)
            return null;
        return { contents: { kind: 'markdown', value: signatureMarkdown(overloads) }, range };
    }
...
const LOCATOR_METHODS = [
    ...
    { label: "getByLabel", kind: node_1.CompletionItemKind.Method, detail: "Get by label", documentation: "Returns locator by label." },
    { label: "getByPlaceholder", kind: node_1.CompletionItemKind.Method, detail: "Get by placeholder", documentation: "Returns locator by placeholder text." },
    { label: "getByAltText", kind: node_1.CompletionItemKind.Method, detail: "Get by alt text", documentation: "Returns locator by alt text." },
    { label: "getByTitle", kind: node_1.CompletionItemKind.Method, detail: "Get by title", documentation: "Returns locator by title." },
    { label: "getByTestId", kind: node_1.CompletionItemKind.Method, detail: "Get by test id", documentation: "Returns locator by test id." }
];
connection.onCompletion((params) => {
    ...
    if (directiveMatch) {
        ...
        return AUTOIT_DIRECTIVES.map(d => ({
            ...
            filterText: d.name,
            textEdit: node_1.TextEdit.replace(range, d.name)
        }));
    }
    ...
    if (macroMatch) {
        ...
        return AUTOIT_MACROS.map(m => ({
            ...
            filterText: m.name,
            textEdit: node_1.TextEdit.replace(range, m.name)
        }));
    }
    // IntelliSense for Locator object methods after .locator(...).
    if (/\.locator\([^)]*\)\.$/i.test(before)) {
        return withCallParens(LOCATOR_METHODS);
    }
